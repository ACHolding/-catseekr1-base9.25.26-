#!/usr/bin/env python3
"""CATSEEK-BASE 1.x — offline bilingual RAM-only BitNet LLM on the Kimi K3s engine.
...
class CatSeekGUI:
    ...
    def _build(self) -> None:
        ...
        catseek.add_command(label="New CATSEEK-BASE chat", command=self._new_chat)
        catseek.add_command(label="CATSEEK-BASE model card", command=lambda: self._submit("/model"))
        catseek.add_command(label="CATSEEK-BASE inference self-test", command=lambda: self._submit("/selftest"))
        catseek.add_separator()
        catseek.add_command(label="Exit CATSEEK-BASE", command=root.destroy)
        ...
        self.status = tk.Label(header, text="CATSEEK-BASE inference ready", bg=self.PANEL, fg=self.BLUE_2, font=self._font(10))
    ...
    def _new_chat(self) -> None:
        ...
        if self.busy:
            ...
            self.status.config(text="CATSEEK-BASE new chat ready")
        self.entry.focus_set()
    ...
    def _submit(self, forced: Optional[str] = None) -> None:
        ...
        self.status.config(text="CATSEEK-BASE computing next tokens…")
        ...
        try:
            while True:
                ...
                if kind == "done" and is_current:
                    ...
                    self.status.config(text=f"{APP_NAME} ready · {reply.tokens_per_second:.1f} tok/s")
                elif kind == "error" and is_current:
                    self._append("ERROR\n" + str(payload) + "\n", "error")
                    self.status.config(text="CATSEEK-BASE error")
